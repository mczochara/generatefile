...
// Check configuration file
        ...
        String[] files = {checkConfiguration.getParameter(parameter[1]), checkConfiguration.getParameter(parameter[2])};
        check.checkConfigFiles(files);

// Call executing method generateResultFile after checking arguments/configuration
        try {
            generateResultFile(checkConfiguration.getParameter(parameter[0]), checkConfiguration.getParameter(parameter[1]), checkConfiguration.getParameter(parameter[2]));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
